# ifndef TTT3D_LEVEL_MASTER_BUTTON_BASE

	# define TTT3D_LEVEL_MASTER_BUTTON_BASE 1

	# include <glm/glm.hpp>
	# include <glm/gtc/matrix_transform.hpp>

	class levelMasterButtonBase {
		...
		public:
			...
			levelMasterButtonBase ( const char * fileNo , loggingHandler * logger = new loggingHandler ) {
				this -> logger = logger ;

				this -> logger -> logInfo ( "levelMasterButtonBase : Initializing button base" ) ;
				this -> fileNo = fileNo ;
				this -> logger -> logInfo ( "levelMasterButtonBase : Setting button object" ) ;
				...
				this -> logger -> logInfo ( "levelMasterButtonBase : Initialized button base\n" ) ;
			}

			~levelMasterButtonBase ( ) {
