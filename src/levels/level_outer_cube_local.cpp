# ifndef TTT3D_LEVEL_OUTER_CUBE

	# define TTT3D_LEVEL_OUTER_CUBE 1
	...
	class levelOuterCubeLocal : public levelBase {
		...
		protected:
			...
			struct cornerLinesOuterCubes                cornerCubes [ 4 ]             ;
			struct centerLinesOuterCubes                centerCubes [ 6 ]             ;
			short unsigned int                          playerNo                      ;
			short int                                   totalPlayers                  ;
			double                                      gameEndTimerStart             ;

			void initializeMap ( ) {
				this -> logger -> logInfo ( "levelOuterCubeLocal : Initializing line points" ) ;
				for ( int i = 0 ; i < 6 ; i ++ ) {
				...
				this -> logger -> logInfo ( "levelOuterCubeLocal : Initializing board" ) ;
				for ( int i = - 1 ; i < 2 ; i ++ ) {
					for ( int j = - 1 ; j < 2 ; j ++ ) {
						for ( int k = - 1 ; k < 2 ; k ++ ) {
							if ( ! ( ( i == j ) && ( j == k ) && ( k == 0 ) ) ) {
				...
				this -> logger -> logInfo ( "levelOuterCubeLocal : Setting total players to 3 and current player as player 1\n" ) ;
				this -> playerNo = 0 ;
			}
			...
			void actionOnMap ( int * x , int * y , int * z ) {
				this -> logger -> logInfo ( "levelOuterCubeLocal : Initializing colorized board" ) ;
				for ( int i = 0 ; i < 3 ; i ++ )
					for ( int j = 0 ; j < 3 ; j ++ )
						for ( int k = 0 ; k < 3 ; k ++ )
							if ( this -> board [ i ] [ j ] [ k ] != NULL )
				...
				this -> logger -> logInfo ( "levelOuterCubeLocal : Checking selected pixel" ) ;
				int pickedColor = this -> inputObject -> getPixelColorUnderMouse ( ) ;

				this -> logger -> logInfo ( "levelOuterCubeLocal : Mapping selected pixel color to its object" ) ;
				for ( int i = 0 ; i < 3 ; i ++ ) {
					for ( int j = 0 ; j < 3 ; j ++ ) {
						for ( int k = 0 ; k < 3 ; k ++ ) {
				...
				this -> logger -> logInfo ( "levelOuterCubeLocal : Clear screen from colorized board\n" ) ;
			...
			int updateMap ( ) {
				this -> logger -> logInfo ( "levelOuterCubeLocal : Checking selected object" ) ;
				int x = ( - 5 ) , y = ( - 5 ) , z = ( - 5 ) ;
				...
				if ( x != ( - 5 ) ) {
					this -> logger -> logInfo ( "levelOuterCubeLocal : Selected object found! Changed to non-selectable block" ) ;
					this -> logger -> logInfo ( "levelOuterCubeLocal : Modifying line points" ) ;
					if ( ( x == 0 ) && ( z == 0 ) )
					...
					this -> logger -> logInfo ( "levelOuterCubeLocal : Checking whether any line points are filled" ) ;
					for ( int i = 0 ; i < 4 ; i ++ ) {
						if ( ( ( this -> cornerCubes [ i ] ) . line1 == ( incrementValue * 3 ) ) ||
							...
							this -> logger -> logInfo ( "levelOuterCubeLocal : A player has won! Notify calling block\n" ) ;
							return 1 ;
						 }
					}
					...
					this -> logger -> logInfo ( "levelOuterCubeLocal : No line pints filled. Switch to next players turn" ) ;
					this -> playerNo = ( ( this -> playerNo + 1 ) % ( this -> totalPlayers ) ) ;
				}

				this -> logger -> logInfo ( "levelOuterCubeLocal : No one has won yet. Going back" ) ;
				return 0 ;
			}

		public:

			levelOuterCubeLocal ( windowHandler * windowObject , inputHandler * inputObject , short int noOfPlayers = 2 , loggingHandler * logger = new loggingHandler )
				: levelBase ( windowObject , inputObject , logger ) {
				this -> logger -> logInfo ( "levelOuterCubeLocal : Initializing outer level\n" ) ;
				this -> fpsObject = new fpsCounter ( this -> logger ) ;
				this -> totalPlayers = noOfPlayers ;
				this -> gameEndTimerStart = ( - 1 ) ;
				this -> logger -> logInfo ( "levelOuterCubeLocal : Initialized outer cube level\n" ) ;
			}
			...
			int main ( ) {
				this -> logger -> logInfo ( "levelOuterCubeLocal : Started outer cube board level" ) ;

				this -> logger -> logInfo ( "levelOuterCubeLocal : Setting up cube board" ) ;
				this -> inputObject -> resetValues ( ) ;
				this -> initializeMap ( ) ;
				int checkPress = 0 , checkEscape = 0 , checkSuccess = 0 ;

				this -> logger -> logInfo ( "levelOuterCubeLocal : Running frames" ) ;
				...
				do {
					...
					if ( ( this -> inputObject -> checkMouseButtonReleased ( GLFW_MOUSE_BUTTON_LEFT ) ) && ( checkPress == 1 ) ) {
						this -> logger -> logInfo ( "levelOuterCubeLocal : Left click triggered! Check what was selected" ) ;
						checkPress = 0 ;
					...
					do {
						if ( checkSuccess == 1 ) {
							...
							char msg [ 200 ] ;
							snprintf ( msg , 200 , "levelOuterCubeLocal : Player %d wins!" , ( ( this -> playerNo ) + 1 ) ) ;
							...
							this -> logger -> logInfo ( "levelOuterCubeLocal : No player wins" ) ;
				...
				this -> logger -> logInfo ( "levelOuterCubeLocal : Destroying cube board" ) ;
				this -> destroyMap ( ) ;

				this -> logger -> logInfo ( "levelOuterCubeLocal : Resetting camera and inputs" ) ;
				this -> inputObject -> resetValues ( ) ;

				this -> logger -> logInfo ( "levelOuterCubeLocal : Ended the outer cube level. Exiting\n" ) ;
				this -> inputObject -> resetValues ( ) ;
				return 0 ;
			}
