# ifndef GL_MODEL_STRUCTURE_HANDLER
	...
	class modelStructureHandler {
		...
		protected:
			...
			int loadOBJ ( const char * filePath , struct modelStructureInfo * modelStructureRetrieved , int invertUV = 0 ) {
		...
		public:
			...
			const static short int                      STRUCT_OBJ             = 0x00 ;

			modelStructureHandler ( const char * fileNo , int type = STRUCT_OBJ , int invertUV = 0 , loggingHandler * logger = new loggingHandler ) {
				this -> logger = logger ;

				char logMsg [ 200 ] = "modelStructureHandler : Initializing the model structure handler for structure \'" ;
				...
				char structureFilePath [ 500 ] = "models/structures/structure-" ;
				strcat ( structureFilePath , fileNo ) ;
				strcat ( structureFilePath , ".st" ) ;
				if ( type == STRUCT_OBJ ) {
					this -> logger -> logInfo ( "modelTextureHandler : Loading structure using OBJ loader" ) ;
					if ( this -> loadOBJ ( structureFilePath , & ( this -> modelStructure ) , invertUV ) ) {
						this -> logger -> logInfo ( "modelStructureHandler : Could not retrieve structure details. Have to exit\n" ) ;
						exit ( - 1 ) ;
					}
				}
				else {
					this -> logger -> logInfo ( "modelStructureHandler : Structure file type not supported. Have to exit\n" ) ;
					exit ( - 1 ) ;
				}
				...
				modelStructuresList [ fileNo ] = this ;
				modelStructuresListCount [ fileNo ] = 1 ;
				this -> fileNo = fileNo ;
			...
			~modelStructureHandler ( ) {
				...
				modelStructuresList . erase ( this -> fileNo ) ;
				modelStructuresListCount . erase ( this -> fileNo ) ;
			}
			...
			void bindIndicesBuffer ( ) {
				glBindBuffer ( GL_ARRAY_BUFFER , ( this -> indicesID ) ) ;
			}
			...
			static modelStructureHandler * checkStructure ( const char * fileNo ) {
				if ( modelStructuresList . count ( fileNo ) == 0 )
					return NULL ;
				else
					return modelStructuresList [ fileNo ] ;
			} 
